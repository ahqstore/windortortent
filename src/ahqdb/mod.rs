...
use crate::ahqdb::ps1::update::run_update_ps1;
...
impl<'a> AHQDBApplication<'a> {
  ...
  pub fn update<T: AsRef<str>>(
    ...
    // Add Type
    fs::write(format!(r"{dir}\ahqdb\type"), match ty {
      Type::AllUsers => "all",
    ...
    fs::write(format!(r"{dir}\ahqdb\.link"), to_string(self.shortcut_info.as_ref().unwrap()).map_err(|_| AHQDBError::InvalidAHQDBFile)?)?;
